# Contribute to MarzbanNext
Thanks for considering contributing to MarzbanNext!
...
- Also tell the version of MarzbanNext, Xray and docker (if you use docker) you are using.
...
### Components Library
The frontend uses [shadcn](https://ui.shadcn.com/) as its primary component library, with Tailwind CSS for styling. Reuse primitives from `app/dashboard/src/components/ui` before creating custom UI components.

When a required shadcn primitive is missing, add it from `app/dashboard` with:

```bash
npm run ui:add -- <component...>
```

The project wrapper uses the registry configured in `components.json` and places generated primitives in `src/components/ui`. Build feature-specific compositions next to the owning feature. Do not introduce new Chakra UI usage; migrate affected legacy surfaces to shadcn when the change scope permits.

Use the shadcn `AlertDialog` component for all action confirmations, especially destructive, irreversible, or otherwise consequential operations. Do not use browser `confirm()`, a regular `Dialog`, or custom confirmation modals.

Strive to create cohesive, single-purpose components. Readability and maintainability are more important than brevity.

## MarzbanNext CLI
MarzbanNext CLI is built using [Typer](https://typer.tiangolo.com/), and its commands' code can be found in `cli`  directory. Its documentation is generated using [Typer CLI](https://typer.tiangolo.com/typer-cli/) which can be re-generated by navigating to project's root directory and running the following command (`typer-cli` package needs to be installed first):
