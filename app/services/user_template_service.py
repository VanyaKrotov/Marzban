...
def add_user_template(
    ...
    """
    try:
        return template_crud.create_user_template(db, new_user_template)
...
def modify_user_template(
    ...
    """
    try:
        return template_crud.update_user_template(db, dbuser_template, modify_user_template)
    except IntegrityError:
        db.rollback()
        raise HTTPException(status_code=409, detail="Template by this name already exists")
    except ValueError as exc:
...
def remove_user_template(
    ...
    return template_crud.remove_user_template(db, dbuser_template)
...
def get_user_templates(
    ...
    return template_crud.get_user_templates(db, offset, limit)
