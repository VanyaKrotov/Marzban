...
IMAGE_REPOSITORY="marzban-local"
RAW_REPOSITORY_URL="https://raw.githubusercontent.com/$PROJECT_REPO"
SCRIPT_URL="$RAW_REPOSITORY_URL/$PROJECT_BRANCH/scripts/marzban.sh"
...
download_project_file() {
    ...
    curl --fail --silent --show-error --location \
        "$FILES_URL_PREFIX/$source_path" -o "$destination"
}
...
get_latest_release_tag() {
    ...
    release_tag=$(curl --fail --silent --show-error \
        "https://api.github.com/repos/$PROJECT_REPO/releases/latest" \
        | jq -r ".tag_name")
...
get_release_architecture() {
    case "$(uname -m)" in
        ...
        arm64 | aarch64)
            ...
            ;;
        *)
            colorized_echo red "No Docker image is published for architecture $(uname -m)."
            return 1
            ;;
    esac
}

install_marzban_image() {
    local marzban_version=$1
    local architecture
    local download_url
    local image_tag="$marzban_version"
    ...
    architecture=$(get_release_architecture)
    if [ "$marzban_version" == "latest" ]; then
        image_tag=$(get_latest_release_tag)
        download_url="https://github.com/$PROJECT_REPO/releases/latest/download/marzban-linux-$architecture.tar.gz"
    else
        download_url="https://github.com/$PROJECT_REPO/releases/download/$marzban_version/marzban-linux-$architecture.tar.gz"
    fi
    ...
    if ! curl --fail --show-error --location --retry 3 \
        ...
        colorized_echo red "Unable to download Docker image from $download_url"
        return 1
    fi
...
install_marzban() {
    ...
    local source_ref="$marzban_version"

    if [ "$source_ref" == "latest" ]; then
        source_ref=$(get_latest_release_tag)
    fi

    FILES_URL_PREFIX="$RAW_REPOSITORY_URL/$source_ref"
    MARZBAN_IMAGE="$IMAGE_REPOSITORY:$marzban_version"
...
EOF
        ...
        download_project_file ".env.example" "$APP_DIR/.env"
...
EOF
        ...
        download_project_file "docker-compose.yml" "$docker_file_path"

        yq -i ".services.marzban.image = \"$MARZBAN_IMAGE\"" "$docker_file_path"
...
install_command() {
    ...
    while [[ $# -gt 0 ]]; do
        ...
        case $key in
            ...
            ;;
            --dev)
                colorized_echo red "Development images are not published. Use a release version."
                exit 1
            ;;
            --version)
    ...
    detect_compose
    install_marzban_script
    # Tags are the source of versioned images and configuration files.
    check_version_exists() {
        ...
        curl --fail --silent --head \
            "$RAW_REPOSITORY_URL/$version/.env.example" >/dev/null
    }
    # Check if the version is valid and exists
    if [[ "$marzban_version" == "latest" || "$marzban_version" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
        if check_version_exists "$marzban_version"; then
            install_marzban_image "$marzban_version"
...
update_marzban() {
    install_marzban_image "latest"
    yq -i ".services.marzban.image = \"$IMAGE_REPOSITORY:latest\"" "$COMPOSE_FILE"
}
