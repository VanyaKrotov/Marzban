...
# Fetch the public address without making every command depend on the service.
NODE_IP=$(curl --silent --max-time 5 -4 ifconfig.io || true)

# If the IPv4 retrieval is empty, attempt to retrieve the IPv6 address
if [ -z "$NODE_IP" ]; then
    NODE_IP=$(curl --silent --max-time 5 -6 ifconfig.io || true)
fi
...
install_marzban_node_script() {
    ...
    curl --fail --silent --show-error --location "$SCRIPT_URL" -o "$TARGET_PATH"
...
usage() {
    ...
    colorized_echo yellow "  install           - Install/reinstall Marzban-node"
    colorized_echo yellow "  update            - Update to latest version"
    colorized_echo yellow "  uninstall         - Uninstall Marzban-node"
    colorized_echo yellow "  install-script    - Install Marzban-node script"
    colorized_echo yellow "  uninstall-script  - Uninstall Marzban-node script"
